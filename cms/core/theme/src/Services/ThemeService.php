...
class ThemeService
{
    ...
    public function activate(string $theme): array
    {
        ...
        if (setting('theme') && $theme == Theme::getThemeName()) {
            return [
                'error' => true,
                'message' => trans('core/theme::theme.theme_activated_already', ['name' => $theme]),
            ];
        }
            ...
            if (!empty($content)) {
                ...
                if (!empty($requiredPlugins)) {
                    foreach ($requiredPlugins as $plugin) {
                        $this->moduleService->activate($plugin);
                    }
                }
            }
        ...
        return [
            'error' => false,
            'message' => trans('core/theme::theme.active_success', ['name' => $theme]),
        ];
    }
    ...
    protected function validate(string $theme): array
    {
        ...
        if (!$this->files->exists($location . '/theme.json')) {
            return [
                'error' => true,
                'message' => trans('core/theme::theme.missing_json_file'),
            ];
        }

        return [
            'error' => false,
            'message' => trans('core/theme::theme.theme_invalid'),
        ];
    }
    ...
    public function publishAssets(string|null $theme = null): array
    {
        ...
        foreach ($themes as $key => $theme) {
            ...
            if (!$this->files->isDirectory($themePath)) {
                ...
                return [
                    'error' => true,
                    'message' => trans('core/theme::theme.folder_is_not_writeable', ['name' => $themePath]),
                ];
            }
            ...
            if (File::exists($this->getPath($theme, 'screenshot.png'))) {
        ...
        return [
            'error' => false,
            'message' => trans('core/theme::theme.published_assets_success', ['themes' => implode(', ', $themes)]),
        ];
    }
    ...
    public function remove(string $theme): array
    {
        ...
        if (Theme::getThemeName() == $theme) {
            return [
                'error' => true,
                'message' => trans('core/theme::theme.cannot_remove_theme', ['name' => $theme]),
            ];
        }
        ...
        return [
            'error' => false,
            'message' => trans('core/theme::theme.theme_deleted', ['name' => $theme]),
        ];
    }
    ...
    public function removeAssets(string $theme): array
    {
        ...
        return [
            'error' => false,
            'message' => trans('core/theme::theme.removed_assets', ['name' => $theme]),
        ];
    }
}
